...
void ASTBuilder::BuildAST(treelite::Model const& model, bool thresh_as_int, bool prob_to_int) {
  main_node_ = AddNode<MainNode>(
      nullptr, model.base_scores.AsVector(), ComputeAverageFactor(model), model.postprocessor, prob_to_int);
      ...
      [&](auto&& model_preset) {
        for (std::size_t tree_id = 0; tree_id < model_preset.trees.size(); ++tree_id) {
          ASTNode* tree_head = BuildASTFromTree(func, model_preset.trees[tree_id],
              static_cast<int>(tree_id), model.target_id[tree_id], model.class_id[tree_id], 0, thresh_as_int, prob_to_int, model.GetNumTree());
...
ASTNode* ASTBuilder::BuildASTFromTree(ASTNode* parent,
    ...
    std::int32_t class_id, int nid, bool thresh_as_int, bool prob_to_int, int num_trees) {
  ...
  if (tree.IsLeaf(nid)) {
    ...
    if (tree.NodeType(nid) == treelite::TreeNodeType::kNumericalTestNode) {
      if (prob_to_int) {
        ast_node = AddNode<NumericalConditionNode>(parent, tree.SplitIndex(nid),
            tree.DefaultLeft(nid), tree.ComparisonOp(nid), static_cast<float>(tree.Threshold(nid)), std::nullopt, thresh_as_int);
      } else {
        ast_node = AddNode<NumericalConditionNode>(parent, tree.SplitIndex(nid),
    ...
    ast_node->children_.push_back(
        BuildASTFromTree(ast_node, tree, tree_id, target_id, class_id, tree.LeftChild(nid), thresh_as_int, prob_to_int, num_trees));
    ast_node->children_.push_back(
        BuildASTFromTree(ast_node, tree, tree_id, target_id, class_id, tree.RightChild(nid), thresh_as_int, prob_to_int, num_trees));
  }
